...
public:
	Function()=default;
	Function(std::vector < std::string >& args,std::vector < std::string >& body):mArgs(args),mBody(body) {}
	Function(const Function&)=default;
	virtual ~ Function()=default;
	bool operator==(const Function & func) const
	{
		return this->mArgs == func.mArgs;
	}
	Variable::Number call(std::vector < cov:: any > &);
};
...
Variable::Var Function::call(std::vector < cov::any > &arglist)
{
	if (arglist.size() != mArgs.size())
		throw std::logic_error("The number of parameter error.");
	for (int i = 0; i < mArgs.size(); ++i)
		Variable::addVar(mArgs[i], arglist[i]);
	Variable::Var result = Expression::parse(mBody);
	for (auto & it:mArgs)
		Variable::delVar(it);
	return result;
}*/
